...
class Data:
    ...
    def load(filename):
        ...
        Data.penality = int(data["penality"])
        Data.capacity = data["capacity"]


        for i in range(len(data["subset"])):
            subset = data["subset"][i]
...
class Individual:
    ...
    def evaluate(self):
        ...
        for i in subset:
            if i not in res:
                ...
                self.fitness -= Data.subset_cost 
            if i not in Data.capacity:
    ...
    def show_info(self):
        print("SOLUÇÃO ------------------------------")
        subset = []
        print("Gene do individuo = ", end="")
        print(self.gene)
        
        for i in range(Data.size):
            if self.gene[i] == 1:
                subset += Data.subset[i]
        print("Subset nao organizado = ", end="")
        print(subset)

        subset.sort()
        print("Subset organizado = ", end="")
        print(subset)

        subset = list(set(subset))
        print("subset sem repeticao = ", end="")
        print(subset)
